...
static ssize_t read_f32(struct bt_conn *conn, const struct bt_gatt_attr *attr,
			void *buf, uint16_t len, uint16_t offset)
{
	const uint16_t *u16 = attr->user_data;
	float value = sys_cpu_to_le16(*u16);
...
int readTempSensor ( struct device *dev, TempValue *t ) {
	int ret ;
	...
	float c = temp_value.val1 + temp_value.val2*.000001 ;
...
void main(void)
{
	...
	#if ( 1 )
		dev = device_get_binding(DT_LABEL(DT_INST(0, ams_as6221)));
	#else
		dev = device_get_binding(DT_LABEL(DT_INST(0, ti_tmp116)));
	#endif

	__ASSERT(dev != NULL, "Failed to get AS6221 device binding");
